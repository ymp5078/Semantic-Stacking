...
class AbdominalDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.domains[0] == 'SABSCT':
            ...
            prompt = "A 2D slice of a abdominal T2-SPIR MRI scan using showing " + ", ".join([LABEL_NAME[c] for c in unique_classes])
...
def get_gen_training(modality,location_scale, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3):
    return AbdominalDataset(idx_pct = idx_pct,\
        mode = 'train',\
        domains = modality,\
        transforms = None,\
