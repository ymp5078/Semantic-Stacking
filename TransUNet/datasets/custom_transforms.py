...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        gen_img = sample.get('gen_image',None)
        # print(img.size)
        w, h = img.size
        if self.padding > 0 or w < self.size[0] or h < self.size[1]:
            ...
            if gen_img is not None:
                gen_img = ImageOps.expand(gen_img, border=padding, fill=0)
        ...
        if w == tw and h == th:
            if gen_img is not None:
                gen_img = gen_img.resize(img.size,Image.BILINEAR)
                return {'image': img,
                        'label': mask,
                        'gen_image':gen_img,
        ...
        if gen_img is not None:
            gen_img = gen_img.crop((x1, y1, x1 + tw, y1 + th))
            sample['gen_image'] = gen_img
...
class RandomScaleCrop(object):
    ...
    def __call__(self, sample):
        ...
        if seed > 0.5:
            ...
            gen_img = sample.get('gen_image',None)
            if gen_img is not None:
                gen_img = gen_img.resize((w, h), Image.BILINEAR)
                sample['gen_image'] = gen_img
...
class Normalize_tf(object):
    ...
    Args:
        ...
        std (tuple): standard deviations for each channel.
    """
    def __init__(self, mean=(0., 0., 0.), std=(1., 1., 1.), fundus=True):
        self.mean = mean
        self.std = std
        self.fundus = fundus
...
def ToMultiLabel(dc,num_domain=3):
    new_dc = np.zeros([num_domain])
...
class ToTensor(object):
    ...
    def __init__(self, num_domain=3) -> None:
    ...
    def __call__(self, sample):
        ...
        gen_img = sample.get('gen_image',None)
        if gen_img is not None:
            gen_img = np.array(gen_img).astype(np.float32).transpose((2, 0, 1))
            gen_img = torch.from_numpy(gen_img).float()
            sample['gen_image'] = gen_img
            
        sample['image']=img
        sample['label']=map
        domain_code = torch.from_numpy(SoftLable(ToMultiLabel(sample['dc'],num_domain=self.num_domain))).float()
