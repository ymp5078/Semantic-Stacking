...
class FCBFormer(nn.Module):
    def __init__(self, size=352):
        ...
        self.out_feature = {}
        self.PH[1].register_forward_hook(self.get_activation('out_feature'))

    def get_activation(self,name):
        def hook(model, input, output):
            self.out_feature[name] = output
        return hook
    
    def forward(self, x, return_features=False):
        x1 = self.TB(x)
        TB_feature = x1
        x2, FCB_features = self.FCB(x)
        ...
        # for LSC loss
        if return_features:
            return out, TB_feature, FCB_features, self.out_feature['out_feature']
        return out
