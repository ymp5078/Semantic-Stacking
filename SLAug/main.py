...
class DataModuleFromConfig(torch.nn.Module):
    ...
    def _train_dataloader(self):
        return DataLoader(self.datasets["train"], batch_size=self.batch_size,
                          num_workers=self.num_workers, shuffle=True, persistent_workers=True)
...
if __name__ == "__main__":
    ...
    try:
        LSC_config = config.pop('local_feature_consistant',OmegaConf.create())
        
        try:
            if LSC_config.mask:
                print('reweight LSC loss base on the segmentation mask.')
        except:
    ...
    for cur_epoch in range(max_epoch):
        if SBF_config.usage:
            cur_iter = train_one_epoch_SBF(model, criterion,train_loader,opt,torch.device('cuda'),cur_epoch,cur_iter, optimizer_config.max_iter, SBF_config, visdir, LSC_config=LSC_config)
        else:
            cur_iter = train_one_epoch(model, criterion, train_loader, opt, torch.device('cuda'), cur_epoch, cur_iter, optimizer_config.max_iter, LSC_config=LSC_config)
        if scheduler is not None:
            scheduler.step()

        # Save Bset model on val
        if (cur_epoch+1)%100==0:
            torch.save({'model': model.state_dict()}, os.path.join(ckptdir,f'epoch_{cur_epoch}.pth'))
            ...
            print(str)
            if data.datasets.get('test') is not None:
                cur_test_dice = evaluate(model, test_loader, torch.device('cuda'))
                # if np.mean(cur_dice)>best_dice:
                #     best_dice=np.mean(cur_dice)
                str=f'Epoch [{cur_epoch}]   '
                for i,d in enumerate(cur_test_dice):
                    str+=f'Class {i}: {d}, '
                str+=f'Test DICE {np.mean(cur_test_dice)}/{best_dice}'
                print(str)
