...
class transform_with_label(object):
    ...
    def __call__(self, comp, c_label, c_img, nclass, is_train, use_onehot = False, use_gen_image = False):
        """
        Args
            comp:               a numpy array with shape [H x W x C + c_label]
            c_label:            number of channels for a compact label. Note that the current version only supports 1 slice (H x W x 1)
        ...
        if is_train is True:
            ...
            t_label_h = np.rint(t_label_h)
            t_img = comp[..., 0 : c_img ]
            # if use_gen_image: # the first C it the generate image
            #     gen_img = t_img[..., 0:1]
            #     t_img = t_img[..., 1 : c_img ]
        ...
        #     # print(t_img.shape,i)
        #     t_img_list.append(self.intensity_tfx(t_img[...,  i]))
        # # if use_gen_image: # the first C is the generate image
        # t_img = np.stack( t_img_list, -1 )
